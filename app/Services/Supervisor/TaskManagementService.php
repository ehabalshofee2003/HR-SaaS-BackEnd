...
class TaskManagementService
{
    ...
    {
        $tasks = $this->repository->list($supervisorId, $filters);

        return array_map(fn($t) => $this->formatListItem($t), $tasks);
    }
    ...
    public function create(int $supervisorId, array $data): array
    {
        ...
        if (!$employee) {
            throw ValidationException::withMessages(['employee_id' => ['Employee not found or not assigned to you.']]);
        }

        $companyId = User::find($supervisorId)?->getCurrentCompanyId();

        if (!$companyId) {
            throw ValidationException::withMessages(['company' => ['Could not determine company.']]);
        }
    ...
    {
        $task = $this->repository->find($id, $supervisorId);
    ...
    private function formatDetails(object $t): array
    {
        return [
            ...
            'employee_id' => $t->employee_user_id,
